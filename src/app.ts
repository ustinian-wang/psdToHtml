import { DesignConfig, EnvConfig, BootConfig, TypeEnum, DesignDeviceEnum } from "./Config";
import { checkFileExists } from "@ustinian-wang/node-kit";
import { PsdDeal } from "./PsdDeal";
import { default as cheerio } from 'cheerio';
import * as fs from "node:fs";
import * as path from "node:path";
import { pathResolver } from "./pathParser";
//工具类
import { Utils } from "./utils/uitls";

const rootPath = path.join(pathResolver.resolveFromRoot("src"), "..");
...
class PsdToHtml {
    ...
    private async saveToDev() {
        ...
        //转成服务器路径
        let pngFilePathList: Array<string> = pngToJpgList.map((pngName: string) => {
            return path.join(config.imgDir, pngName);
        });

        //把样式里面的图片引用给替换掉
        let cssContent: string = this.replaceCssPngToJpg(result.css, pngToJpgList);
        cssContent += funcResult.css; //基础css和功能css放到同一个文件中
    ...
    private async copyToProd() {
        ...
        //拷贝样式
        //将开发环境的css路径替换成生成环境
        const cssImgBase = `/image/${projectName}/`;
        ...
        //复制图片
        Utils.copyDir(devEnvConfig.imgDir, config.imgDir).then(() => {
            console.log("图片复制完毕");
        });
        Utils.writeFile(config.css, cssContent).then(() => {
            console.log("样式复制完毕");
        });
        Utils.writeFile(config.html, htmlData).then(() => {
            console.log("html复制完毕");
        });
    }
    ...
    private replaceCssPngToJpg(css: string, pngNames: Array<string>) {
        pngNames.forEach((pngName) => {
            ...
            const regExp: RegExp = new RegExp(pngName, "g");
    ...
    private async getPngsForPngToJpg(dir: string) {
        ...
        let bigSizeImgArr: Array<string> = srcFiles.filter((pngName: string) => {
            let pngPath: string = path.join(dir, pngName);
            let fileStats: any = fs.statSync(pngPath);
            const SIZE_LIMIT = 1024 * 150; //500K

            return fileStats.size >= SIZE_LIMIT;
        });
        //判断图片是否半透明
        let allTask: Array<Promise<{ pngName: string, isOpacity: boolean }>> = bigSizeImgArr.map((pngName: string) => {
            let pngPath: string = path.join(dir, pngName);
            //异步判断图片是否存在透明区域
            return Utils.is_png_exist_opacity(pngPath, 1).then((isOpacity) => {
                return {
                    pngName: pngName,
                    isOpacity: isOpacity
                };
        ...
        //过滤出最终结果
        let allTaskInfo: Array<{ pngName: string, isOpacity: boolean }> = await Promise.all(allTask);

        return allTaskInfo.filter((info) => {
...
export async function run(config: any) {
    ...
    let res = await checkFileExists(psdFilePath);
...
function getDesignConf(type: number): DesignConfig {
    let design750Config: DesignConfig = {
        device: DesignDeviceEnum.MOBILE,
        selfAdapt: false,
        width: 750,
        funcList: []
    };
    let design2400Config: DesignConfig = {
        device: DesignDeviceEnum.PC,
        selfAdapt: true,
        width: 2400,
        funcList: []
    };
